...
**New to the team? Read [`docs/OPERATING_MODEL.md`](./docs/OPERATING_MODEL.md) first.**
Then walk this accelerator layer by layer in [`docs/EXAMPLE.md`](./docs/EXAMPLE.md), and
use [`docs/GATES.md`](./docs/GATES.md) to know what "done" means at each phase.

> **Building or running this app? The authoritative build/run guide is
> [`docs/APP_GUIDE.md`](./docs/APP_GUIDE.md)** (formerly this repo's root README), with the
> demo path in [`docs/presentations/README_DEMO.md`](./docs/presentations/README_DEMO.md)
> and the FSI runbook in
> [`docs/presentations/demo-runbook-fsi.md`](./docs/presentations/demo-runbook-fsi.md).
...
├── infra/         install_deps.py · build_frontend.py · VAST/NEXUS probes · docker/
...
|-----------|------------|------|
| **A way to choose** | Sourcing + weighted scoring (≥ 4.0 / 5 advances) | [§1–2](./docs/OPERATING_MODEL.md#1-how-we-source-use-cases) · [scorecard](./docs/business-case/scorecard.md) |
| **A build process** | 6-phase stage-gate (Discover → Publish) | [§3–4](./docs/OPERATING_MODEL.md#3-the-development-process) |
| **A build standard** | One reference stack: Ingest → Lakehouse → Process → AI → Serve | [§5–6](./docs/OPERATING_MODEL.md#5-the-build-standard) · [architecture](./docs/architecture/README.md) |
| **A standard deliverable** | This repo — a working accelerator, deployable as a CML AMP | [§7](./docs/OPERATING_MODEL.md#7-the-standard-deliverable-this-repo) |
...
```bash
python infra/install_deps.py       # pinned Python deps (GPU-aware)
python infra/build_frontend.py     # user-local Node + Vite build → app/frontend/dist
...
python infra/vast_probe.py         # VAST S3 metadata calls
python infra/nexus_probe.py        # NEXUS LTM endpoint
```
...
- **`tfm_demo/` + `src/`** — the inference engine: GPU tokenizer + TFM decoder →
  ...
  ([design](./docs/nexus-ltm-design.md)).
...
- **`infra/`** — pinned installs, the frontend toolchain, VAST/NEXUS connectivity probes,
  and the Docker path (`infra/docker/` + root compose).
