...
export default function RunsTable({ runs }: Props) {
  return (
    <div className="bg-surface-2 rounded-lg border border-surface-3 p-4">
      <h3 className="text-sm font-medium text-gray-300 mb-2">Run history</h3>
      {runs.length === 0 ? (
              ...
              {[...runs].reverse().map((r) => {
                ...
                return (
                  <tr key={r.run_id} className="border-b border-surface-4/50 hover:bg-surface-3/50">
                    <td className="py-1.5 px-2 font-mono text-gray-200">#{r.run}</td>
                    ...
                    <td className="py-1.5 px-2 text-center">
                      {r.registry.deployed ? (
                        ...
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-brand-blue/10 text-brand-blue-soft">
